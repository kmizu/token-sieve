...
```sh
codex plugin marketplace add kmizu/token-sieve
codex plugin add token-sieve@token-sieve
```

This repository includes its own marketplace manifest. Codex fetches the repository and installs the plugin; no manual clone, extra directory, or JSON file is needed. In `token-sieve@token-sieve`, both the plugin and its marketplace are named `token-sieve`.

Check the installation:

```sh
codex plugin list --marketplace token-sieve
```
...
If `codex plugin` is unavailable, update the Codex CLI or use the standalone commands below. If Python is installed as `python` rather than `python3`, substitute that executable. Run installation in the same environment as Codex. WSL is the tested environment; native Windows has not been verified.

### Uninstall

```sh
codex plugin remove token-sieve@token-sieve
codex plugin marketplace remove token-sieve
```

Captured logs are retained. Delete them separately only when you no longer need them.

## Standalone use

The same CLI works without installing the Codex plugin:

```sh
git clone https://github.com/kmizu/token-sieve.git
cd token-sieve
```
